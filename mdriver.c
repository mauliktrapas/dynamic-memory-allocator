...
int tts;
   ...
   tts= fscanf(tracefile, "%u", &(trace->sugg_heapsize)); /* not used */
    tts=fscanf(tracefile, "%u", &(trace->num_ids));     
  tts=  fscanf(tracefile, "%u", &(trace->num_ops));     
    tts=fscanf(tracefile, "%u", &(trace->weight));        /* not used */
	...
	switch(type[0]) {
	case 'a':
	tts=    fscanf(tracefile, "%u %u", &index, &size);
